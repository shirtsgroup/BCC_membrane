...
def initialize():
    ...
    parser.add_argument('--tau_p', default=10, type=float, help='Pressure coupling time constant')
...
class SimulationMdp(object):
    ...
    def __init__(self, gro, title='MD Simulation', T=300, em_steps=-1, time_step=0.002, length=1000,
                 ...
                 xlink=False, bcc=False, tau_p=20, tau_t=1, nstxout=5000, nstvout=5000, nstfout=5000, nstenergy=5000,
                 frames=None):
        """
    ...
    def write_em_mdp(self, out='em'):
        ...
        emsteps = 'emstep = %s' % self.time_step
        nsteps = 'nsteps = %s' % self.em_steps
        nstlog = 'nstlog = 40'

        cutoff_scheme = 'cutoff-scheme = verlet'
        pbc = 'pbc = xyz'
        verlet_tol = 'verlet-buffer-tolerance = 0.005'
        eps_r = 'epsilon_r = 10'

        coulomb = 'coulombtype = reaction-field'
        rcoulomb = 'rcoulomb = 1.1'
        vdw_type = 'vdw_type = cutoff'
        vdw_mod = 'vdw_modifier = Potential-shift-verlet'
        rvdw = 'rvdw = 1.1'

        gen_vel = 'gen_vel = no'
        constraints = 'constraint-algorithm = Lincs'
        lincs_order = 'lincs-order = 12'
        lincs_iter = 'lincs-iter = 8'
    ...
    def write_npt_mdp(self, out='npt', length=None):
        ...
        a.append(['cutoff-scheme = verlet\n'])  # I think verlet is default
        a.append(['pbc = xyz\n'])
        a.append(['verlet-buffer-tolerance = 0.005\n'])
        ...
        a.append(['nstxout-compressed = %s\n' % self.nstxout])
        a.append(['nstfout = %s\n' % self.nstfout])
        a.append(['nstenergy = %s\n' % self.nstenergy])
        a.append(['nstlist = 40\n'])  # potential inputs in the future vvv
        a.append(['vdwtype = cutoff\n'])
        a.append(['vdw-modifier = Potential-shift-verlet\n'])
        a.append(['rvdw = 1.1\n'])

        a.append(['epsilon_r = 10\n'])
        a.append(['coulombtype = reaction-field\n'])
        a.append(['rcoulomb = 1.1\n'])
        ...
        if self.xlink:
            ...
            a.append(['lincs-iter=2'])  # I don't think this is necessary
        if self.restraints:
