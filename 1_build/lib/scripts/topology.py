...
class ReadItp(object):
    ...
    def __init__(self, name):
        ...
        try:
            ...
            try:
                f = open('lib/topologies/%s.itp' % name, 'r')
            except FileNotFoundError:
...
class Residue(ReadItp):
    ...
    def __init__(self, name, connectivity=False):
        ...
        with open('lib/topologies/ions.txt') as f:
            ions = []
            for line in f:
                if line[0] != '#':
    ...
    def __init__(self, name):
        ...
        if name in ions:
            ...
            try:
                ...
                try:
                    t = md.load('lib/topologies/%s.pdb' % (name), standard_names=False)  # check if the configuration is
...
class LC(ReadItp):
    ...
    def __init__(self, name):
        ...
        a = []
        with open('lib/topologies/%s.gro' % (name)) as f:
            for line in f:
                a.append(line)

        self.t = md.load("lib/topologies/%s.gro" % (name))
...
class Solute(Residue):
    ...
    def __init__(self, name):
        ...
        self.gro = []
        with open('lib/topologies/%s.gro' % (name), 'r') as f:
            for line in f:
