...
class BicontinuousCubicBuild(topology.LC):
    ...
    def place_monomers(self, shift=0):
        ...
        for i in range(round(self.nmon/2)):
        ...
        for i in range(round(self.nmon/2), self.nmon):
            n = -surfaces.gradient(self.grid[i, :], self.space_group,
                                  period=self.period)  # vector normal to surface at point grid[i, :]
