...
; EDITED 6/17/21 TO BE COMPATIBLE WITH GROMACS 2018 on Bridges2
cutoff-scheme = Group
...
# Set up for running
G="mpirun -np 1 gmx_mpi grompp -f $mdp -c $GRO -n $NDX -p $OTP -o $BASE -maxwarn 2"
echo $G; $G || exit

# Run
M="mpirun -np 8 gmx_mpi mdrun -deffnm $BASE -v -nt $NP -ntomp 1"
echo $M; $M
...
# Run
M="mpirun -np 8 gmx_mpi mdrun -deffnm $BASE -v -nt $NP"
echo $M; $M
...
__MDP__
  ...
  # Set up run
  G="mpirun -np 1 gmx_mpi grompp -f $mdp -c $GRO -r $BW -p $OTP -o $BASE -maxwarn 2"
  echo $G; $G || exit
...
do
  ...
  m=${mdp%.mdp}
  tag=$((++i))-md-${m##*/}
  G="mpirun -np 1 gmx_mpi grompp -f $mdp -c $GRO -r $BW -p $OTP -o $tag -maxwarn 2"
  echo $G; $G || exit
  M="mpirun -np 8 gmx_mpi mdrun -deffnm $tag -v -nt $NP -ntomp 1"
  echo $M; $M
  GRO=$tag.gro
