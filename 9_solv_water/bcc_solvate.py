...
if __name__ == "__main__":
    ...
    equil.remove_solvent('xlinked.gro')
    equil.generate_mdps(length=50, frames=2, T=sim_params['temperature'], xlink=True)  # creates an object
    equil.mdp.write_em_mdp(out='em')
    equil.add_solvent('HOH', out='solvated_water.gro', scale=0.7)  # add that water

    #clean
    if not os.path.isdir("./temp-water"):
        os.mkdir('temp-water')
    mv = "mv solvated_nvt* solvated_npt* npt_equil* npt_* nvt_equil* em_* nvt*.gro solv_*.gro temp-water"
